<script lang="ts">
	...
	const minTempDate = data.find((d) => d[1] == minValue) ?? 0;
	const maxTempDate = data.find((d) => d[1] == maxValue) ?? 0;
...
<svg {width} {height}>
	...
	<g transform="translate(0, ${height})">
		...
		<g transform="translate(0, -5)">
			<text x={timeScale(minDate)} y={height} text-anchor="start"
				>{d3.timeFormat("%b %Y")(minDate)}</text
			>
			<text x={timeScale(maxDate)} y={height} text-anchor="end"
				>{d3.timeFormat("%b %Y")(maxDate)}</text
			>
		</g>
	</g>
	...
	<!-- Minimum and Maximum temperature red dot-->
